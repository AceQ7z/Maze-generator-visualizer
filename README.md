# 🐭 Maze Visualizer
A Python maze generator and solver using pygame, with real-time animation and controls. Generates a Unique random Maze each time and finds the shortest path to the end.
...
- ### Dead end / Back tracking
  when the mouse reaches a dead end, the *ncell* will be empty, naturaly. Thus when this happens, an if statement fires which does the oposite of what the first check1 does fires, ie, it does **current cell path - neighbouring cell path is equal to -1** , it adds the previous step it has taken to *ncell*, and thus upon selection, the mouse travels backwards, retracks. This will take place again and again, ie, the *ncell* returns empty in the check1 as it also checks for another factor before adding the option to the *ncell*(which i had forgot to mention earlier), it checks if the cell it might travel to next is already in a list called *pathway* which basically contains all the cell it had travelled to before. so during check1, even after back tracking once, it will still elimate the cell option infront of mouse from *ncell* so it doesnt keeep going back and forth. this back tracking tackes place till the mouse reaches a cell where another option is availabe, ie, a turn it hadn't took. in this case, the *ncell* will have the value of the cell it hadnt took, so the if statement for back traking wont fire.

- ### End
  And with these logics, the maze generation till the solving takes place till the mouse reaches the end where thee program stops, or becomes stationary. As a hightlight, a list called *shr_path* tracks the shortest path from start end which we can veiw :D


## Provided features via gui :
- I made three buttons that the user can use, for a touch of interactivity. :P
- A **Pause** button, to well, pause thee simulation at any moment.
